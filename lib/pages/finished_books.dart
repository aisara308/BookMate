...
class _FinishedBooksScreenState extends State<FinishedBooksScreen> {
  ...
  Future<void> _loadFinishedBooks() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          allBooks = books.where((b) => b['isFinished'] == true).toList();
          _applySearch();
        });
      } else {
        setState(() {
  ...
  Future<void> toggleFinished(String title) async {
    try {
      ...
      if (response.statusCode == 200) {
        await _loadFinishedBooks(); // заново подгружаем только завершенные книги
      } else {
  ...
  Future<void> toggleFavorite(String title) async {
    try {
      ...
      if (response.statusCode == 200) {
        await _loadFinishedBooks(); // также подгружаем только завершенные книги
      } else {
                    ...
                    children: [
                      ...
                      Expanded(
                        child: filteredBooks.isEmpty
                            ? Center(child: Text(tr(Keys.noFinishedBooksFound)))
                            : ListView.builder(
